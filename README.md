...
| 핀 질감 | 의미 |
|---|---|
| 빈틈 없이 꽉 찬 덩어리 | 현재 조작 중인 블록 |
| 칸마다 떨어진 작은 점 | 이미 고정된 블록 |
| 가로로 이어진 선 | 현재 블록의 예상 착지 위치 |
| X자 질감 | 퍼즐 블록을 놓을 수 없는 위치 |
| 열의 윗면만 표시 | 촉각 표면 스캔 모드 |
| 상단 좌우 경고점 | 낙하 모드 위험 높이 |

가장 중요한 정보인 **현재 블록이 가장 강한 질감**을 갖습니다. 낙하 모드 셀은 4×3핀뿐이라
그 크기에서 "테두리"는 12핀 중 10핀이 올라간 상태여서 손끝에는 그냥 꽉 찬 덩어리로 읽힙니다.
그래서 현재 블록은 셀 경계 없이 전체를 채워 조각 하나가 **이어진 한 덩어리**로 만져지게 하고,
고정 블록은 칸마다 떨어진 작은 점으로 낮춰 대비를 3배로 벌렸습니다.
